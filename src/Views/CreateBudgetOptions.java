...
import classes.DatabaseReader;
import classes.DatabaseWriter;
...
public class CreateBudgetOptions extends JDialog {
	...
	private ArrayList<String> oplatyList = new ArrayList<>();
	private JCheckBox chckbxFunduszRemontowy;
	private JLabel lblErrorPanel2;
	private String errorMessageKategorie = "Uzupe³nij wszystkie zaznaczone kategorie";
	private int numKatOplaty = 0;
	private HashMap<String, String> oplatyKatFromTxtFieldMap = new HashMap<>();
	private ArrayList<String> wydatkiList = new ArrayList<>();
	private HashMap<String, String> wydatkiKatFromTextFieldMap = new HashMap<>();
	private int numKatWydatki = 0;
	private JLabel lblErrorPanel3;
	private HashMap<String, String> oszczednosciFromTxtFieldMap = new HashMap<>();
	private ArrayList<String> oszczednosciList = new ArrayList<>();
	...
	public CreateBudgetOptions() {
				...
				public void keyPressed(KeyEvent e) {
					if (e.getKeyCode() == KeyEvent.VK_ENTER) {
						...
						if(!budgetNameIsUnique){
							JOptionPane.showMessageDialog(null, "Podana nazwa ju¿ istnieje");
						}if(textFieldIsEmpty){
							JOptionPane.showMessageDialog(null, "Wpisz nazwê bud¿etu");
						}else{
		...
		btnZakoñcz3 = new JButton("Zako\u0144cz");
		btnZakoñcz3.setEnabled(false);
		btnZakoñcz3.setPreferredSize(new Dimension(80, 25));
		panelButton3.add(btnZakoñcz3);
	...
	private void createUsers() {
		...
		try {
				...
				jTextFields[i].addKeyListener(new KeyAdapter() {
					...
					public void keyReleased(KeyEvent e) {
						...
						if(userNameIsUnique != 1){
							JOptionPane.showMessageDialog(null, "Nazwa u¿ytkownika musi byæ unikalna");
						}
			...
			if (num > maxNumberOfUsersInBudget) {
				lblErrorMessageUserNumber.setText("Budzet mo¿e mieæ maksymalnie 8 uzytkowników");
				panel1_Users.removeAll();
				btnDalej1.setEnabled(false);
			}

		} catch (NumberFormatException nfe) {
			lblErrorMessageUserNumber.setText("Wpisz liczbê ca³kowit¹");
			panel1_Users.removeAll();
			btnDalej1.setEnabled(false);
		} catch (NegativeArraySizeException excpt) {
			lblErrorMessageUserNumber.setText("Wpisz liczbê ca³kowit¹ wiêksz¹ od zera");
			panel1_Users.removeAll();
			btnDalej1.setEnabled(false);
		}
